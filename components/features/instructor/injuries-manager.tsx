...
import {
  createInstructorInjuryAction,
  deleteInstructorInjuryAction,
  updateInstructorInjuryAction,
...
function InjuryFields({
  ...
  return (
    <>
      ...
      <div className="space-y-1.5">
        <Label htmlFor={`incidentDate-${injury?.id ?? 'new'}`}>
          Incident date
        </Label>
        <Input
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <select
            id={`severity-${injury?.id ?? 'new'}`}
            name="severity"
            className={controlClass}
            defaultValue={injury?.severity ?? 'Minor'}
          >
        ...
        <div className="space-y-1.5">
          ...
          <select
            id={`status-${injury?.id ?? 'new'}`}
            name="status"
            className={controlClass}
            defaultValue={injury?.status ?? 'Under Treatment'}
          >
...
export function InstructorInjuriesManager({
  injuries,
  artists,
}: {
  injuries: AdminInjury[];
  artists: AdminArtist[];
}) {
  return (
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create, update, and resolve injury records.
          </p>
        </div>
        <EntityDialog
          trigger={
          ...
          action={createInstructorInjuryAction}
        >
            ...
            accessor: (row) => (
              <div className="flex items-center gap-1">
                <EntityDialog
                  trigger={
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      aria-label={`Edit injury for ${row.artist_name}`}
                    >
                      <IconEdit />
                    </Button>
                  }
                  title="Edit injury"
                  description="Update incident details or recovery status."
                  submitLabel="Save changes"
                  pendingLabel="Saving…"
                  successMessage="Injury updated."
                  action={updateInstructorInjuryAction}
                ...
                <DeleteDialog
                  ...
                  entity="injury"
                  action={deleteInstructorInjuryAction}
                  trigger={
                    <Button
                      variant="destructive"
                      size="icon-sm"
                      aria-label={`Delete injury for ${row.artist_name}`}
                    >
                      <IconTrash />
                    </Button>
                  }
